"""Sprint 130b: q=3 (3-state Potts) chi_F FSS cross-check -- CALIBRATION.

Same four estimators as 130a. q=3: nu=5/6 -> 1/nu=1.2, 2/nu-d = 2*(6/5)-1 = 1.40
(= our "exact" q=3 chi_F value), g_c=1/3 exact, no marginal operator -> clean.
Second calibration point; if height and collapse recover 1.40 and 1.2 here, the
q=4 application (130c) is trustworthy. CPU-only.
"""
import numpy as np
...
KAPPA0 = 1.2                       # nu=5/6
THEORY_INV_NU = 1.2
THEORY_A = 2.0 / (5.0 / 6.0) - 1.0   # = 1.40
X_LO, X_HI, M = -2.0, 2.0, 17

OUT = os.path.join(os.path.dirname(__file__), '..', 'results', 'sprint_130b_fss_q3.json')
results = {
    'experiment': '130b_fss_q3', 'sprint': 130, 'q': Q, 'g_c': G_C,
...
print("=" * 78)
print(f"Sprint 130b: q={Q} chi_F FSS calibration (g_c={G_C:.4f}, expect 1/nu=1.2, a=1.40)")
print("=" * 78)
...
print(f"\n  (1) at-g_c  : a = {fit_g['alpha']:.4f} +/- {fit_g['alpha_err']:.4f}  "
      f"(theory {THEORY_A:.3f})  R2={fit_g['r_squared']:.5f}")
print(f"  (2) height  : a = {fit_h['alpha']:.4f} +/- {fit_h['alpha_err']:.4f}  "
      f"(theory {THEORY_A:.3f})  R2={fit_h['r_squared']:.5f}")
...
record(sprint=130, model='sq', q=Q, n=nmax, quantity='chiF_height_exp', value=float(a_h),
       error=float(fit_h['alpha_err']), method='chi_F_peakheight', notes=f'theory 2/nu-d={THEORY_A:.3f}')
