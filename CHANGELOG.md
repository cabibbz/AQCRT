...
## Sprint 142 -- Write-up sprint: S136-141 arc consolidated into a paper-style manuscript
No new compute; consolidation. **unresolved/manuscript_EP_walking_arc.md** (draft v1): abstract; intro situating the arc against GRZ/CNPV/Buffenoir-Wallon/Borgs-Janke; methods (EP estimator with the asymmetric-crossing form, charge-conserving DMRG with validation numbers 1e-6..3e-4, the fitting-discipline rules); six results sections (crossover + q=6 null control; the blind q=7 prediction; the asymptotic constant; the velocity test and two-length decoupling; q=8 universality; q=5-only shadow validity); discussion centered on the OPEN PUZZLE -- a quantum tunneling rate locked across q to half the exact classical Borgs-Janke product with both duality transfers experimentally eliminated -- with three candidate resolutions and the fixed-BC quantum interface-free-energy measurement named as the decisive probe; limitations; reproducibility appendix (claim -> script -> data). **Figures (exp_142a_figures.py -> unresolved/figures/fig1-4 png+pdf):** fig1 produced a bonus visual result -- plotted against L/xi_d^cl the q=7/8/10 Dm*L curves nearly COLLAPSE onto a single master trajectory (the Lambda ∝ xi law seen as a scaling collapse) while the q=6 control stays flat; fig3 shows the q=8+q=10 plateaus oscillating about 1/4 with the excluded 0.40/0.50 levels; fig4 pairs the two-length decoupling with the Coulomb-gas shadow anchor. Candidate polish items logged (bootstrap errors, BC dependence, pinning 1/4 vs 0.213). unresolved/sprint_142.md.

## Sprint 141 -- q=8 universality test: the 1/4 constant RECURS -- law, not numerology
The decisive test left by S139/S140. Registered BEFORE data (sprints/sprint_141.md): under H_law, the q=8 local decay rate σ_loc·ξ_d^cl must flatten at 0.25±0.04 by L/ξ≈2 (P1) and tail fits must be consistent with 1/4 while disfavoring 0.40 (frozen onset) and 0.50 (classical duality) (P2). Extended q=8 to n=36,40,44,48 (L/ξ=1.51-2.01; exp_137b harness SPRINT_NO=141; 2-2.9h/size, chi=128, ~10.5h; Δm·L 1.481→1.306; relres 9e-3..4.1e-2 -- n=40 noisiest). **P1 PASSED as written:** tail-window mean (L/ξ≥1.3) = **0.256** vs the q=10 plateau **0.259** -- the same plateau to 1% across a 2.26x change in ξ; the L/ξ≈2 window reads 0.277 ∈ 0.25±0.04. **P2:** the q=8-only tail fit is UNDERPOWERED (4 pts, s=0.71±0.52 -- reported honestly, discriminates nothing); the **joint q=8+q=10 shared-s fit delivers: s = 0.213±0.029** -- 1/4 inside 1.3σ, s=0.40 excluded at 6.4σ, s=0.50 at 9.9σ, s≡1/4 within +0.9 dAIC of free. **VERDICT: σ̃(q)·ξ_d^cl(q) ≈ 1/4 is q-INDEPENDENT** -- S139's constant upgraded from possibly-numerological to a law candidate of the quantum-classical coexistence correspondence. Precision caveat stated plainly: whether the shared constant is exactly 1/4 or ~0.21-0.25 requires larger L/ξ or theory; established are its universality and scale. Standing puzzle (the project's sharpest): the quantum OBC tunneling rate locks to HALF the exact classical Borgs-Janke tension per CLASSICAL correlation length while the quantum chain's own ξ_x (2-4 sites) is decoupled and both duality explanations are experimentally removed. POTENTIALLY NOVEL (strengthened, two-q). Next: the S136-141 write-up sprint + a direct quantum interface-free-energy measurement (fixed-BC DMRG) as the theory probe. DB: sigma_xi_product (q=8); im_gEP_open etc q=8 n=36-48 (sprint 141). unresolved/sprint_141.md.
